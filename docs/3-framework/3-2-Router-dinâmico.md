...
Um router dinâmico é capaz de aceitar rotas que não podem ser previstas.

- `/curso/:slug`  
  slug será o valor dinâmico, ele poderá conter qualquer valor passado pelo usuário.

- `params`  
  O valor de `:slug` deverá ser disponibilizado no `Request`.
